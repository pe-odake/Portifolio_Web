# replit_auth.py - Integration code for Replit Auth
import jwt
import os
import uuid
...
class UserSessionStorage(BaseStorage):
    ...
    def get(self, blueprint):
        try:
            # First try with current user (if logged in)
            ...
            # During OAuth flow, user might not be logged in yet
            # Try to find by browser session key only
            oauth_record = db.session.query(OAuth).filter_by(
                browser_session_key=g.browser_session_key,
                provider=blueprint.name,
            ).first()
    ...
    def set(self, blueprint, token):
        try:
            # During OAuth callback, user might not be logged in yet
            user_id = current_user.get_id() if current_user.is_authenticated else None
            
            # Remove any existing tokens for this session/provider
